...
def load_model_and_processor(device):
    ...
    """
    # Use the local pretrained model directory
    directory_path = "./pretrained_model"

    # Check if the directory exists and is not empty
    if not os.path.exists(directory_path) or not os.listdir(directory_path):
        raise FileNotFoundError(f"Pretrained model directory not found or empty: {directory_path}")

    # Load the model directly from the directory
    model = Mask2FormerForUniversalSegmentation.from_pretrained(directory_path).to(
        device
    )
